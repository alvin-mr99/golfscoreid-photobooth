...
import { printScoreService } from '../services/PrintScoreService';
...
export function PrintScoreButton({ 
  scoreData, 
  selectedPhotos,
  selectedPlayerIds,
  scoreMode = 'stroke',
  ...
  const handleShowPreview = async () => {
    try {
      ...
      // Generate preview HTML with selected players, limited photos, and score mode
      const html = await printScoreService.generatePreviewHTML(scoreData, photosToPreview, selectedPlayerIds, scoreMode);
      setPreviewHTML(html);
    } catch (err) {
  ...
  const handlePrint = async () => {
    try {
      ...
      // Print score and photos with selected players, limited photos, and score mode
      await printScoreService.printScoreAndPhotos(scoreData, photosToPrint, selectedPlayerIds, scoreMode);
  ...
  return (
    <div>
      ...
      {/* Print Preview Modal */}
      {showPreview && (
        <div className="fixed inset-0 bg-black bg-opacity-90 z-50 overflow-y-auto animate-fade-in">
          <div className="min-h-screen flex items-start justify-center p-4 pt-8">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl my-8 flex flex-col animate-scale-in">
            {/* Modal Header */}
            ...
            <div className="p-4 bg-gray-50 flex-shrink-0">
              {isLoadingPreview ? (
                ...
                <div className="bg-white rounded-xl shadow-lg p-4 mx-auto" style={{ maxWidth: '1100px' }}>
            ...
            <div className="p-4 border-t border-gray-200 bg-white rounded-b-3xl flex-shrink-0">
              <div className="flex gap-3">
                <button
                  onClick={handleClosePreview}
                  disabled={isPrinting}
                  className="flex-1 px-6 py-3 bg-gray-200 text-gray-800 rounded-xl font-bold text-base
                ...
                <button
                  ...
                  className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-bold text-base
